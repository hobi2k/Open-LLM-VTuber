...
class VoiceRecognition(ASRInterface):
    ...
    def _create_recognizer(self):
        if self.model_type == "transducer":
            ...
            output_dir = "./models"
            bundled_model = self.sense_voice.startswith(
                "./models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17"
            )
            archive = Path(output_dir) / url.split("/")[-1]
            if bundled_model and archive.exists():
                logger.warning(
                    "Found an unfinished SenseVoice installation. Recovering it."
                )
                check_and_extract_local_file(url, output_dir)
            ...
            try:
                ...
                if not bundled_model:
                    raise
                logger.warning("SenseVoice model is invalid. Reinstalling it once.")
                download_and_extract(url, output_dir, force=True)
                recognizer = sherpa_onnx.OfflineRecognizer.from_sense_voice(
                    model=self.sense_voice,
                    tokens=self.tokens,
                    num_threads=self.num_threads,

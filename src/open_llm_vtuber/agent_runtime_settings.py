...
async def _cli_connection_payload(config: CLIAgentConfig) -> dict:
    ...
    if not resolved:
        return {"available": False, "version": None, "error": "Executable not found"}
...
async def runtime_settings_payload(context: ServiceContext) -> dict:
    ...
    opencode_status, claude_status, codex_status, hermes_status = await asyncio.gather(
        ...
        _cli_connection_payload(claude_code),
        _cli_connection_payload(codex),
        _cli_connection_payload(hermes),
    )
    ...
    return {
        ...
        "claude_code": {**_cli_payload(claude_code), "connection": claude_status},
        "codex": {**_cli_payload(codex), "connection": codex_status},
        "hermes": {**_cli_payload(hermes), "connection": hermes_status},
    }
...
async def runtime_connection_payload(
    ...
    statuses = await asyncio.gather(
        opencode_connection_payload(opencode),
        _cli_connection_payload(CLIAgentConfig(**settings.claude_code.model_dump())),
        _cli_connection_payload(CLIAgentConfig(**settings.codex.model_dump())),
        _cli_connection_payload(CLIAgentConfig(**settings.hermes.model_dump())),
    )
    return dict(
